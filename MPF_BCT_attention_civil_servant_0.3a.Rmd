...
Mean Annual Return: ~10.23%      
Annualized Standard Deviation: ~9.08% (StdDev(monthly return) * sqrt(12))   
Sharpe Ratio (Mean Annual Return): 10.23%/9.08% = 1.1273    
Sortino Ratio: 0.7001 (MAR = 0%)  
Expected Shortfall: 4.66% loss (0% Risk-free rate, 95% C.I.)   
Deflated Sharpe Ratio (p-value): >99.999%     

Monthly installment:    
Total contribution: 337500    
Latest asset value: 928034.8    
Mean annual return: 9.04%      
Internal Rate of Return (IRR): 9.40%    
Annualized Standard Deviation: 9.06%    
Sharpe Ratio: 0.9974       
Sortino Ratio: 0.6381 (MAR = 0%)    
Expected Shortfall: 4.73% loss (0% Risk-free rate, 95% C.I.)     
...
MPF.BCT.returns <- merge(MPF.BCT.AE, MPF.BCT.CA, MPF.BCT.CEHK, MPF.BCT.CT, MPF.BCT.EU, MPF.BCT.E3, MPF.BCT.E5, MPF.BCT.E7, MPF.BCT.E9, MPF.BCT.FM, MPF.BCT.GB, MPF.BCT.GE, MPF.BCT.GT, MPF.BCT.HKB, MPF.BCT.HSIT, MPF.BCT.MPFC, MPF.BCT.RMBB, MPF.BCT.SFP, MPF.BCT.SE40)

MPF.BCT.original.cost <- c(0.0182, 0.0085, 0.0166, 0.0115, 0.0165, 0.0163, 0.0163, 0.0162, 0.0152, 0.0136, 0.0150, 0.0167, 0.0100, 0.0112, 0.0084, 0.0094, 0.0126, 0.0082, 0.0150) / 12
MPF.BCT.cs.cost <- c(0.0069, 0.0062, 0.0069, 0.0060, 0.0069, 0.0062, 0.0062, 0.0062, 0.0062, 0.0062, 0.0055, 0.0069, 0.0060, 0.0055, 0.0050, 0.0040, 0.0055, 0.0062, 0.0062) / 12

for(col in 1:length(MPF.BCT.returns[1,])) {
  MPF.BCT.returns[,col] <- MPF.BCT.returns[,col] - MPF.BCT.cs.cost[col] + MPF.BCT.original.cost[col]
}


monthly <- index(MPF.BCT.returns)
#monthly <- length(monthly)
colnames(MPF.BCT.returns) <- c("MPF.BCT.AE", "MPF.BCT.CA", "MPF.BCT.CEHK", "MPF.BCT.CT", "MPF.BCT.EU", "MPF.BCT.E3", "MPF.BCT.E5", "MPF.BCT.E7", "MPF.BCT.E9", "MPF.BCT.FM", "MPF.BCT.GB", "MPF.BCT.GE", "MPF.BCT.GT", "MPF.BCT.HKB", "MPF.BCT.HSIT", "MPF.BCT.MPFC", "MPF.BCT.RMBB", "MPF.BCT.SFP", "MPF.BCT.SE2040")

rm(MPF.BCT.AE, MPF.BCT.CA, MPF.BCT.CEHK, MPF.BCT.CT, MPF.BCT.EU, MPF.BCT.E3, MPF.BCT.E5, MPF.BCT.E7, MPF.BCT.E9, MPF.BCT.FM, MPF.BCT.GB, MPF.BCT.GE, MPF.BCT.GT, MPF.BCT.HKB, MPF.BCT.HSIT, MPF.BCT.MPFC, MPF.BCT.RMBB, MPF.BCT.SFP, MPF.BCT.SE40)
...
for (pass in 1:30) {
  ...
  for (col in colNum) {
    ...
    len <- 1
    if (length(X) < 62) {
      predicted <- rep(0, length(1:(length(X)-1)))
    } else {
      loop <- length(X) - 1
      #for (i in X_old:(length(X)-1)) {
      #for (loop in 62:length(X) - 1) {
        # X_train <- X[1:(i)]
          # y_train <- y[1:(i)]
        ...
        #  dataset_batch(batch_size, drop_remainder = FALSE)
        
        #validation_dataset <-
        #  tensor_slices_dataset(keras_array(list(X_test, y_test))) %>%
        #  dataset_batch(batch_size, drop_remainder = FALSE)
        ...
        attention <-
          function(tensor_units = 128L,
            ...
            keras_model_custom(name = name, function(self) {
               ...
               self$lstm <- layer_lstm(units = tensor_units,
                  return_sequences = TRUE,
                  return_state = TRUE,
                  stateful = TRUE,
                  recurrent_initializer = 'glorot_normal',
                  dtype='float32')
              self$Wh <- layer_dense(units = tensor_units,
                  dtype='float32')
              self$Ws <- layer_dense(units = tensor_units,
                  dtype='float32')
              self$Wx <- layer_dense(units = 1L,
                  dtype='float32')
              self$V <- layer_dense(units = 1L,
                  dtype='float32')
              self$Output <- layer_dense(units = output_size,
                  dtype='float32')
              
              self$call <- function(inputs, mask = NULL) {
                input <- inputs[[1]]
                hidden <- inputs[[2]]
                state <- inputs[[3]]
                
                hidden <- tf$expand_dims(hidden, 1L)
                input <- tf$expand_dims(input, 0L)
                
                print(input)
                print(hidden)
                print(state)
                
                score <- self$V(tensorflow::tf$nn$tanh(self$Wx(input) + self$Wh(hidden)+ self$Ws(state)))
                
                
                attention.weights <- tensorflow::tf$nn$softmax(score, axis=1L)
                
                
                c(encoder_outputs, state_h, state_c) %<-% self$lstm(input*attention.weights)
                output = self$Output(state_h)
                
                list(attention.weights, state_h, state_c, output)
              }
              
              self$init_hidden_state <- function() {
                hidden <- k_zeros(c(batch_size, tensor_units))
                state <- k_zeros(c(batch_size, tensor_units))
                list(hidden, state)
              }
        ...
        split_seq <- function(sequence_c, n_steps = 120) {
          ...
          for (i in 1:length(sequence_c)) {
            end_ix <- i + n_steps
            if (end_ix > length(sequence_c)) {
              break
            }
            seq_x <- sequence_c[i:(end_ix-1)]
            seq_y <- sequence_c[end_ix]
            X <- c(X, seq_x)
            y <- c(y, seq_y)
          }
            X_mat <- matrix(X, nrow = n_steps, byrow = TRUE)
            y_mat <- matrix(y, byrow = FALSE)
            list(X=X_mat, y=y_mat)
        }
        
        X_train <- split_seq(X[1:loop], 60)$X
        y_train <- split_seq(y[1:loop], 60)$y
  
        X_test <- split_seq(X[1:(loop+1)], 60)$X
        y_test <- split_seq(y[1:(loop+1)], 60)$y
        
        
        # dim(X_train) <- c(60,length(X_train[,1]))
        # dim(X_test) <- c(60,length(X_test[,1]))
        # dim(X_train) <- c(1,60,length(X_train[,1]))
        # dim(X_test) <- c(1,60,length(X_test[,1]))
        dim(X_train) <- c(as.integer(length(X_train[1,])),60L)
        dim(X_test) <- c(as.integer(length(X_test[1,])),60L)
  
        attention.model <- attention(tensor_units = 128L, batch_size = batch_size, output_size = 1L)
        optimizer <- keras::optimizer_adam(lr = learning)
        
              
        
        training <- function(model, input, target, hidden, state) {
          loss <- 0
          prediction <- 0

          with(tape <- tf$GradientTape(), {
            for (i in 1:length(input)) {
              c(attention_weight, hidden, state, output) %<-% model$call(list(tf$cast(input, tf$float32),hidden,state))
              loss <- loss + tf$keras$losses$mean_squared_error(tf$cast(target, tf$float32), output)
              prediction <- prediction + 1
            }
          })
          variables <- model$trainable_variables
          grads <- tape$gradient(loss, variables)
          optimizer$apply_gradients(purrr::transpose(list(grads, variables)))
          loss/prediction
        }
        
        epochs <- 22
        early.stopping <- c(100)
        patience <- c(2,0)
        ...
        for (epoch in 1:epochs) {
          starting <- Sys.time()
          c(hidden, state) %<-% attention.model$init_hidden_state()
          loss <- training(attention.model, X_train, y_train, hidden, state)
          early.stopping <- c(early.stopping, as.numeric(tail(loss,1)))
          if (early.stopping[epoch+1] >= early.stopping[epoch]) {
            patience[2] <- patience[2] + 1
          }
          if (patience[1] <= patience[2]) {
            print('Epoch ' %+% as.character(epoch) %+% '. Loss:'%+% as.character(as.numeric(tail(loss,1))))
          }

          if (epoch%%2 ==0) {
            print('Epoch ' %+% as.character(epoch) %+%'. Loss: ' %+% as.character(as.numeric(tail(loss,1))))
            print('Time for 1 epoch need ' %+% as.character((Sys.time()-starting)))
          }
        }

        evaluation <- function(model, data) {
          c(hidden, state) %<-% model$init_hidden_state()
          prediction <- c()
          for (i in 1:length(data)) {
            c(attention_weight, hidden, state, output) %<-%
              model$call(list(tf.cast(data[i,], tf$float32),hidden,state), NULL)
            prediction <- c(prediction, output)
          }
        ...
        #   X_test <- rollapply(X[1:(loop+1)], FUN = function(x) {na <- rep(NA,4); if (length(x) == 4) {for (i in (5-length(x)):4) {na[i] <- x[i]}; na} else {for(i in length(x):1) {na[length(x)-i+1] <- x[i]}; rev(na)}}, width = 4, partial = TRUE, fill = NA, align = "right")
        #   
        #   
        # predicted[len] <- tail(evaluation(attention.model, X_test), 1)
        # len <- len + 1
        #   
        # }
...
for (col in 1:length(MPF.BCT.w[1,])) {
  MPF.BCT.w[,col] <- ((MPF.BCT.w[,col]+1)/2*(max_return[col] - min_return[col]) + min_return[col])
}
...
c("MPF.BCT.AE", "MPF.BCT.CA", "MPF.BCT.CEHK", "MPF.BCT.CT", "MPF.BCT.EU", "MPF.BCT.E3", "MPF.BCT.E5", "MPF.BCT.E7", "MPF.BCT.E9", "MPF.BCT.FM", "MPF.BCT.GB", "MPF.BCT.GE", "MPF.BCT.GT", "MPF.BCT.HKB", "MPF.BCT.HSIT", "MPF.BCT.MPFC", "MPF.BCT.RMBB", "MPF.BCT.SFP", "MPF.BCT.SFP", "MPF.BCT.SE2040")
...
#   for (col in 1:length(MPF.BCT.w[1,])) {
#     MPF.BCT.w.i[,col] <- ((MPF.BCT.w.i[,col]+1)/2*(max_return[col] - min_return[col]) + min_return[col])
#   }
