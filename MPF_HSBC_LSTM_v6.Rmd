...
Annualized Return: ~8.97% (after deducting fees)  
Mean Annual Return: ~9.02% (after deducting fees)   
Annualized Standard Deviation: ~10.93% (StdDev(monthly return) * sqrt(12))   
Sharpe Ratio (Mean Annual Return): 9.02%/10.93% = 0.8248   
Sortino Ratio: 0.4063 (MAR = 0%)  
Expected Shortfall: 7.20% loss (0% Risk-free rate, 95% C.I.)   
Deflated Sharpe Ratio (p-value): 99.7%     
...
for (pass in 1:30) {
  ...
  model %>% layer_lstm(units = 7, return_sequences=TRUE, , 
                             input_shape = c(1,1), activation="tanh") %>%
    layer_lstm(units = 7, activation="tanh", return_sequences=TRUE) %>%
    layer_lstm(units = 7, activation="tanh") %>%
    layer_dense(1)
...
for (row in 1:length(MPF.HSBC.w[, 1])) {
  ...
  if (row <= 12 || MPF.HSBC.w.sum == MPF.HSBC.w[row, 10] ||
    ...
    MPF.HSBC.p[row, 10] <- 1
             ...
             min(MPF.HSBC.stock.return[(row-3):row]) < -0.07) {
    if (row >= 110) {
  ...
  if (tail(na.omit(MPF.portf.drawdown), 1) < -0.05 &&
      up == FALSE) {
    hedge = TRUE
  }
...
for (pass in 1:30) {
  ...
  for (col in 1:length(MPF.HSBC.w[1, ])) {
    ...
    MPF.portf.drawdown <- Drawdowns(MPF.portf.return,
                                  geometric = TRUE)
    if (row > 12 && tail(na.omit(MPF.portf.drawdown), 1) < -0.05 &&
        up == FALSE) {
      hedge = TRUE
    }
  }
