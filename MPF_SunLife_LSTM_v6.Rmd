...
Annualized Return: ~10.99% (after deducting fees)  
Mean Annual Return: ~10.29% (after deducting fees)   
Annualized Standard Deviation: ~9.96% (StdDev(monthly return) * sqrt(12))    
Sharpe Ratio (Annualized Return): 10.99%/9.96% = 1.103   
Sharpe Ratio (Mean Annual Return): 10.29%/9.96% = 1.034   
Sortino Ratio: 0.6292 (MAR = 0%)  
Expected Shortfall: 5.67% loss (0% Risk-free rate, 95% C.I.)   
...
```{r warning=FALSE}
setwd("~")
MPF.SunLife <- as.xts(read.zoo("D:/OneDrive/MPF/Sun Life Rainbow/SunLife.csv", format="%d/%m/%Y", header=TRUE, read = read.csv, na.strings="0"))
...
for (pass in 1:30) {
  ...
  for (col in colNum) {
    ...
    for (i in 1:length(X)-1) {
      ...
      if (i != length(X) - 1) {
        history <- model %>% fit(X_train, y_train, validation_data = list(X_test, y_test), epochs=22, batch_size = 128)
        #plot(history,metrics=c('loss'))
      } else {
        history <- model %>% fit(X_train, y_train, epochs=22, batch_size = 64)
...
for (row in 1:length(MPF.SunLife.w[, 1])) {
  ...
  if (row > 8 && MPF.SunLife.stock.return[row] <
      quantile(na.omit(MPF.SunLife.stock.return), c(.15)) &&
      MPF.SunLife.stock.return[row - 3] <
      quantile(na.omit(MPF.SunLife.stock.return), c(.30))) {
    up <- FALSE
  }
  ...
  if (row > 8 && min(MPF.SunLife.stock.return[(row-6):row]) > -0.03) {
    up <- TRUE
    hedge <- FALSE
  }
  ...
  } else if (min(MPF.SunLife.stock.return[(row-3):row]) < -0.06) {
