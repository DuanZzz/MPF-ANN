...
| ----------------- | -------- | ------------ |   
| 0.0013            | 0.08296  | 0.07129      |   
| 0.0002            | 0.08398  | 0.07127      |   
| 0.0005            | 0.08139  | 0.07114      |   
| 0.0012            | 0.08262  | 0.07095      |   
...
Annualized Return: ~10.32% (after deducting fees)  
Mean Annualized Return: ~11.41% (after deducting fees)   
Annualized Standard Deviation: 10.60% (StdDev(monthly return) * sqrt(12))   
Sharpe Ratio: 10.58%/11.41% = 0.9988  
Sortino Ratio: 0.5399 (MAR = 0%)  
Expected Shortfall: 5.87% loss (0% Risk-free rate, 95% C.I.)  

--------------------------------------------------------------------------------

Benchmark: 50-50 as described in the book written by Mr. Leung   
(before deducting cost?)   
Mean Annualized Return: 10.33%   
  ...
  if (col != 11) {
    for (row in 1:length(MPF.SunLife.RSI.month[,col])) {
      ...
      if (percentile(MPF.SunLife.RSI.month[row,col]) >= (RSI_Overbuy - ((length(1:row)^(1/3))/(length(1:row)^(1/2))))) {
        MPF.SunLife.RSI.p[row,col] <- 0.0001
      } else {
        MPF.SunLife.RSI.p[row,col] <- 1.0001-(percentile(MPF.SunLife.RSI.month[row,col])^2)
      }
    }
...
for (row in 1:length(MPF.SunLife.w[,1])) {
  ...
  for (col in 1:length(MPF.SunLife.w[1,])) {
    ...
    if (col != 7 && col != 9 && col != 11 && col != 12) {
      ...
      if (MPF.SunLife.w[row,col] < Min_NMMA) {
  ...
  if (MPF.SunLife.stock.return[row] < 0 && MPF.SunLife.stock.return[row-3] < -0.004) {
    up <- FALSE
  }
  
  if (hedge == TRUE && MPF.SunLife.stock.return[row] > 0 && 
      MPF.SunLife.stock.return[row-3] > -0.002) {
    hedge <- FALSE
    up <- TRUE
  }
  ...
  # due to the composition of SunLife Fund are too active
  # the NMMA BIAS are triggered too frequently
  # See explanation before
  
  
  if (MPF.SunLife.w.sum < 0.000025 || hedge == TRUE) {
