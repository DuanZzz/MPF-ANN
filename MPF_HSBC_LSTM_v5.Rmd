---
title: "HSBC MPF Investment Strategy and Backtesting (Preliminary Trial)"
output: 
  html_document:
    df_print: paged
...
Annualized Return: ~9.88% (after deducting fees)  
Mean Annual Return: ~10.02% (after deducting fees)   
Annualized Standard Deviation: ~12.22% (StdDev(monthly return) * sqrt(12))   
Sharpe Ratio (Mean Annual Return): 10.02%/12.22% = 0.8199   
Sortino Ratio: 0.4260 (MAR = 0%)  
Expected Shortfall: 7.24% loss (0% Risk-free rate, 95% C.I.)   
Deflated Sharpe Ratio (p-value): 99.8%     
...
# Apply the managment cost in return
for (col in 1:length(MPF.HSBC.returns[1,])) {
  MPF.HSBC.returns[,col] <- MPF.HSBC.returns[,col] - MPF.HSBC.cost[col]
}
...
# Apply the managment cost in return
for (col in 1:length(MPF.HSBC.returns[1,])) {
  MPF.HSBC.dailyReturns[,col] <- MPF.HSBC.dailyReturns[,col] - MPF.HSBC.dailyCost[col]
}
...
for (pass in 1:30) {
  ...
  model %>% layer_cudnn_lstm(units = 7, return_sequences=TRUE, , 
                             input_shape = c(1,1)) %>%
    layer_cudnn_lstm(units = 7, return_sequences=TRUE) %>%
    layer_cudnn_lstm(units = 7) %>%
    layer_dense(1)
  ...
  for (col in colNum) {
    ...
    for (i in 1:length(X)-1) {
      ...
      if (i != length(X) - 1) {
        history <- model %>% fit(X_train, y_train, validation_data = list(X_test, y_test), epochs=22, batch_size = 128, verbose = 0)
        #plot(history,metrics=c('loss'))
      } else {
        history <- model %>% fit(X_train, y_train, epochs=22, batch_size = 128, verbose = 0)
      ...
      return <- model %>% predict(input,batch_size = 128)
...
VIX[VIX >= 35] <- 39.9
VIX.month <-
  do.call(rbind, lapply(split(na.fill((
    1 - (VIX / 40)
  ), 0), "months"), function(x)
    colAvgs(x)))
...
for (row in 1:length(MPF.HSBC.w[, 1])) {
  ...
  for (col in 1:length(MPF.HSBC.w[1, ])) {
    ...
    MPF.HSBC.w[row, col] <-
      na.fill((MPF.HSBC.w[row, col]) * MPF.HSBC.RSI.p[row, col], 0) * VIX.month[row, 6]
    
    if (col != 7 && col != 10 && col != 12) {
      if (!is.na(MPF.HSBC.returns.mat[row, col]) && MPF.HSBC.returns.mat[row,col] != 0) {
        MPF.HSBC.stock.mean <-
  ...
  if (row >= 5 && MPF.HSBC.stock.return[row] < 0.005 &&
      MPF.HSBC.stock.return[row - 3] < 0) {
    up <- FALSE
  }
  
  if (hedge == TRUE && row >= 5 &&
      MPF.HSBC.stock.return[row] > 0.005  &&
      MPF.HSBC.stock.return[row - 3] > 0) {
    hedge <- FALSE
    up <- TRUE
  }
...
for (pass in 1:30) {
  ...
  for (col in 1:length(MPF.HSBC.w[1, ])) {
    ...
    MPF.HSBC.w.i[row, col] <-
      na.fill((MPF.HSBC.w.i[row, col]) * MPF.HSBC.RSI.p[row, col], 0) * VIX.month[row, 6]
