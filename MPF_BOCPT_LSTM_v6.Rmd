...
Mean Annual Return: ~9.76%   
Annualized Standard Deviation: ~10.11% (StdDev(monthly return) * sqrt(12))    
Sharpe Ratio (Mean Annual Return): 9.76%/10.11% = 0.9649      
Sortino Ratio: 0.5013 (MAR = 0%)  
Expected Shortfall: 6.23% loss (0% Risk-free rate, 95% C.I.)   
Deflated Sharpe Ratio (p-value): 99.97%     

Monthly installment:   
Total contribution: 337500    
Latest asset value: 917490.7      
Mean annual return: 9.48%   
Internal Rate of Return (IRR): 9.30%       
Annualized Standard Deviation: 10.10%   
Sharpe Ratio: 0.9376   
Sortino Ratio: 0.5073 (MAR = 0%)   
Expected Shortfall: 6.22% loss (0% Risk-free rate, 95% C.I.)   

--------------------------------------------------------------------------------

Benchmark: 50-50 as described in the book written by Mr. Leung   
Mean Annual Return: 6.11%   
Annualized Standard Deviation: 12.68% (StdDev(monthly return) * sqrt(12))   
Sharpe Ratio: 6.11%/12.68% = 0.4819   

--------------------------------------------------------------------------------

Benchmark: 10th strategy by Mr. Leung   
Mean Annual Return: 10.23%   
Annualized Standard Deviation: 9.11% (StdDev(monthly return) * sqrt(12))   
Sharpe Ratio: 10.23%/9.11% = 1.123   
...
MPF.BOCPT.AE <- dailyReturn(as.xts(MPF.BOCPT$AE), type="log")
MPF.BOCPT.Bond <- dailyReturn(as.xts(MPF.BOCPT$Bond), type="log")
MPF.BOCPT.CA <- dailyReturn(as.xts(MPF.BOCPT$CA), type="log")
MPF.BOCPT.CE <- dailyReturn(as.xts(MPF.BOCPT$CE), type="log")
MPF.BOCPT.ET <- dailyReturn(as.xts(MPF.BOCPT$ET), type="log")
MPF.BOCPT.GE <- dailyReturn(as.xts(MPF.BOCPT$GE), type="log")
MPF.BOCPT.HKE <- dailyReturn(as.xts(MPF.BOCPT$HKE), type="log")
MPF.BOCPT.HKT <- dailyReturn(as.xts(MPF.BOCPT$HKT), type="log")
MPF.BOCPT.JE <- dailyReturn(as.xts(MPF.BOCPT$JE), type="log")
MPF.BOCPT.MPFC <- dailyReturn(as.xts(MPF.BOCPT$MPFC), type="log")
MPF.BOCPT.NAT <- dailyReturn(as.xts(MPF.BOCPT$NAT), type="log")
MPF.BOCPT.SFP <- dailyReturn(as.xts(MPF.BOCPT$SFP), type="log")
MPF.BOCPT.G <- dailyReturn(as.xts(MPF.BOCPT$G), type="log")
MPF.BOCPT.B <- dailyReturn(as.xts(MPF.BOCPT$B), type="log")
MPF.BOCPT.S <- dailyReturn(as.xts(MPF.BOCPT$S), type="log")


MPF.BOCPT.dailyReturns <- merge(MPF.BOCPT.AE, MPF.BOCPT.Bond, MPF.BOCPT.CA, MPF.BOCPT.CE, MPF.BOCPT.ET, MPF.BOCPT.GE, MPF.BOCPT.HKE, MPF.BOCPT.HKT, MPF.BOCPT.JE, MPF.BOCPT.MPFC, MPF.BOCPT.NAT, MPF.BOCPT.SFP, MPF.BOCPT.G, MPF.BOCPT.B, MPF.BOCPT.S)
...
max_return <- colMaxs(MPF.BOCPT.returns)
min_return <- colMins(MPF.BOCPT.returns)

MPF.BOCPT.returns_normalized <- MPF.BOCPT.returns
MPF.BOCPT.returns_normalized[,] <- 0

for (col in 1:length(MPF.BOCPT.returns[1,])) {
  MPF.BOCPT.returns_normalized[,col] <- (MPF.BOCPT.returns[,col] - min_return[col]) / (max_return[col] - min_return[col])
}
...
for (col in 1:length(MPF.BOCPT.w[1,])) {
  MPF.BOCPT.w[,col] <- (MPF.BOCPT.w[,col] * (max_return[col] - min_return[col])) + min_return[col]
}
...
for (row in 1:length(MPF.BOCPT.w[, 1])) {
  ...
  for (col in 1:length(MPF.BOCPT.w[1, ])) {
    ...
    if (col != 2 && col != 3 && col != 10 && col != 12 && col != 14 && col != 15) {
      if (!is.na(MPF.BOCPT.returns.mat[row, col]) && MPF.BOCPT.returns.mat[row,col] != 0) {
  ...
  if (row <= 12 || MPF.BOCPT.w.sum == MPF.BOCPT.w[row, 11] ||
    ...
    if (row >= 30) {
      ...
      MPF.BOCPT.p[row, 2] <- 0.47
      MPF.BOCPT.p[row, 10] <- 0.2    
    } else {
      MPF.BOCPT.p[row, ] <- MPF.BOCPT.w[row, ] / MPF.BOCPT.w.sum / 3
      MPF.BOCPT.p[row, 10] <- 0.67
    }
