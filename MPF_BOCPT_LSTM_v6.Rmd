...
Annualized Standard Deviation: ~9.16% (StdDev(monthly return) * sqrt(12))    
Sharpe Ratio (Mean Annual Return): 9.45%/9.16% = 1.031   
Sortino Ratio: 0.5422 (MAR = 0%)  
Expected Shortfall: 5.57% loss (0% Risk-free rate, 95% C.I.)   
...
```{r warning=FALSE}
setwd("~")
MPF.BOCPT <- as.xts(read.zoo("D:/OneDrive/MPF/BOCPT/BOCPT.csv", format="%d/%m/%Y", header=TRUE, read = read.csv, na.strings="0"))
...
for (row in 1:length(MPF.BOCPT.w[, 1])) {
  ...
  } else if (min(MPF.BOCPT.stock.return[(row-3):row]) < -0.06) {
    if (row >= 30) {
