...
Mean Annual Return: ~12.09%   
Annualized Standard Deviation: ~9.86% (StdDev(monthly return) * sqrt(12))    
Sharpe Ratio (Mean Annual Return): 12.09%/9.86% = 1.2265   
Sortino Ratio: 0.7737 (MAR = 0%)  
Expected Shortfall: 5.37% loss (0% Risk-free rate, 95% C.I.)   
Deflated Sharpe Ratio (p-value): >99.999%     

Monthly installment:   
Total contribution: 324000       
Latest asset value: 1193684         
Mean annual return: 12.08%    
Internal Rate of Return (IRR): 12.26%    
Annualized Standard Deviation: 9.84%    
Sharpe Ratio: 1.2288      
Sortino Ratio: 0.7725 (MAR = 0%)   
Expected Shortfall: 5.37% loss (0% Risk-free rate, 95% C.I.)   
  ...
  if (row <= 12 || MPF.SunLife.w.sum == MPF.SunLife.w[row, 11] ||
    ...
    if (row >= 102) {
      ...
      MPF.SunLife.p[row, 11] <- 1
             ...
             min(MPF.SunLife.stock.return[(row-3):row]) < -0.07) {
    MPF.SunLife.p[row, ] <- MPF.SunLife.w[row, ] / MPF.SunLife.w.sum / 3
    MPF.SunLife.p[row, 11] <- MPF.SunLife.p[row, 11] + 0.67
  } else {
...
for (pass in 1:30) {
  ...
  MPF.SunLife.w.i[MPF.SunLife.w.i==0] <- NA
  for (col in 1:length(MPF.SunLife.w[1,])) {
    MPF.SunLife.w.i[,col] <- ((MPF.SunLife.w.i[,col]+1)/2*(max_return[col] - min_return[col]) + min_return[col])
  }
...
MPF.monthly.asset <- as.xts(rowSums(MPF.SunLife.returns), order.by = monthly)
MPF.monthly.asset[] <- 0
...
for (row in 1:length(MPF.SunLife.p[,1])){
  if (row == 1) {
    last.value <- 1500
    this.value <- as.numeric(exp(as.matrix(MPF.SunLife.returns[row,11])) * 1500)
