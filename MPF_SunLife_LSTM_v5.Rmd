...
Percentage on ANN: 100%   

Annualized Return: ~12.46% (after deducting fees)  
Mean Annual Return: ~13.97% (after deducting fees)   
Annualized Standard Deviation: ~12.38% (StdDev(monthly return) * sqrt(12))   
Sharpe Ratio (Mean Annual Return): 13.97%/12.38% = 1.128   
Sortino Ratio: 0.5192 (MAR = 0%)  
Expected Shortfall: 7.33% loss (0% Risk-free rate, 95% C.I.)   
Deflated Sharpe Ratio (p-value): 99.9%     
...
```{r warning=FALSE}
setwd("~")
MPF.SunLife <- as.xts(read.zoo("D:/OneDrive/MPF/Sun Life Rainbow/SunLife.csv", format="%Y/%m/%d", header=TRUE, read = read.csv, na.strings="0"))
...
MPF.SunLife.SMA.return <- apply(MPF.SunLife.returns, 2, function(x) SMA(x,1))
...
max_return <- colMaxs(MPF.SunLife.returns)
min_return <- colMins(MPF.SunLife.returns)

MPF.SunLife.returns_normalized <- MPF.SunLife.returns
MPF.SunLife.returns_normalized[,] <- 0

for (col in 1:length(MPF.SunLife.returns[1,])) {
  MPF.SunLife.returns_normalized[,col] <- (MPF.SunLife.returns[,col] - min_return[col]) / (max_return[col] - min_return[col])
}
...
for (col in 1:length(MPF.SunLife.w[1,])) {
  MPF.SunLife.w[,col] <- (MPF.SunLife.w[,col] * (max_return[col] - min_return[col])) + min_return[col]
}
...
MPF.SunLife.stock.return <-
  as.xts(rowSums(MPF.SunLife.RSI.month), order.by = monthly)
...
for (row in 1:length(MPF.SunLife.w[, 1])) {
  ...
  for (col in 1:length(MPF.SunLife.w[1, ])) {
    MPF.SunLife.w[row, col] <-
      na.fill((MPF.SunLife.w[row, col]) * MPF.SunLife.RSI.p[row, col], 0) * VIX.month[row, 6]
  ...
  if (MPF.SunLife.stock.return[row] < 0.002 &&
      MPF.SunLife.stock.return[row - 3] < -0.004) {
    up <- FALSE
  }
  
  if (hedge == TRUE &&
      MPF.SunLife.stock.return[row] > 0.002  &&
      MPF.SunLife.stock.return[row - 3] > -0.004) {
    hedge <- FALSE
    up <- TRUE
  }
  ...
  if (MPF.SunLife.w.sum == MPF.SunLife.w[row, 11] ||
      MPF.SunLife.w.sum < 0.0002 || hedge == TRUE) {
...
for (pass in 1:30) {
  ...
  MPF.SunLIfe.returns.mat <- as.matrix(MPF.SunLife.w.all[,,pass])
  ...
  for (row in 1:length(MPF.SunLife.w.i[, 1])) {
    ...
    for (col in 1:length(MPF.SunLife.w.i[1, ])) {
      MPF.SunLife.w.i[row, col] <-
        na.fill((MPF.SunLife.w.i[row, col]) * MPF.SunLife.RSI.p[row, col], 0) * VIX.month[row, 6]
      ...
      if (col != 7 && col != 9 && col != 11 && col != 12) {
        ...
        if (MPF.SunLife.w.i[row, col] < 1e-6) {
    ...
    if (MPF.SunLife.w.sum == MPF.SunLife.w.i[row, 11] ||
        MPF.SunLife.w.sum < 0.0002 || hedge == TRUE) {
      MPF.SunLife.p[row, 11] <- 1
    } else if (length(which(MPF.SunLife.w.i[row, ] != 0)) == 1) {
