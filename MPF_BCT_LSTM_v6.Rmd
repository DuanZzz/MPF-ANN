---
title: "BCT MPF Investment Strategy and Backtesting (Preliminary Trial)"
output: 
  html_document:
    df_print: paged
...
Mean Annual Return: ~9.18%      
Annualized Standard Deviation: ~8.72% (StdDev(monthly return) * sqrt(12))   
Sharpe Ratio (Mean Annual Return): 9.18%/8.72% = 1.052    
Sortino Ratio: 0.6184 (MAR = 0%)  
Expected Shortfall: 4.65% loss (0% Risk-free rate, 95% C.I.)   
...
```{r warning=FALSE}
setwd("~")
MPF.BCT <- as.xts(read.zoo("D:/OneDrive/MPF/BCT/BCT.csv", format="%d/%m/%Y", header=TRUE, read = read.csv, na.strings="0"))
  ...
  if (row <= 12 || MPF.BCT.w.sum == MPF.BCT.w[row, 16] ||
    ...
    if (row >= 24) {
      ...
      MPF.BCT.p[row, 16] <- 1
             ...
             min(MPF.BCT.stock.return[(row-3):row]) < -0.08) {
    if (row >= 24) {
...
charts.PerformanceSummary(portf.rebal.fm, methods= "ModifiedES", geometric=TRUE, p=.95, main="BCT MPF Scheme Performance")
