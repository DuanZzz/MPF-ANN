...
Mean Annual Return: ~10.22%      
Annualized Standard Deviation: ~9.09% (StdDev(monthly return) * sqrt(12))   
Sharpe Ratio (Mean Annual Return): 10.22%/0.09% = 1.1237    
Sortino Ratio: 0.6984 (MAR = 0%)  
Expected Shortfall: 4.67% loss (0% Risk-free rate, 95% C.I.)   
Deflated Sharpe Ratio (p-value): >99.96%     

Monthly installment:    
Total contribution: 337500    
Latest asset value: 925627.2    
Mean annual return: 9.02%      
Internal Rate of Return (IRR): 9.38%    
Annualized Standard Deviation: 9.07%    
Sharpe Ratio: 0.9939       
Sortino Ratio: 0.6363 (MAR = 0%)    
...
for (col in 1:length(MPF.BCT.returns[1,])) {
  MPF.BCT.returns_normalized[,col] <- ((MPF.BCT.returns[,col] - min_return[col]) / (max_return[col] - min_return[col])) * 2 - 1
}
...
for (pass in 1:30) {
  ...
  model %>% layer_lstm(units = 64, activation = "tanh", return_sequences=TRUE, 
                      input_shape = c(1,1), kernel_initializer = initializer_glorot_normal(seed[pass])) %>%
